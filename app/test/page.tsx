...
function Item({ index, position, scale, c = new THREE.Color(), ...props }) {
    ...
    //@ts-ignore
    return <Image ref={ref} {...props} position={position} scale={10} onClick={click} onPointerOver={over} onPointerOut={out} />
}
...
function Items({ w = 0.7, gap = 0.15 }) {
    ...
    return (
        <ScrollControls horizontal damping={0.1} pages={(width - xW + urls.length * xW) / width}>
            <Minimap />
            <Scroll>
                {urls.map((url, i) => <Item key={i} index={i} position={[i * xW, 0, 0]} scale={[w, 4, 9]} url={url} />) /* prettier-ignore */}
            </Scroll>
        </ScrollControls>
    )
}
